<?php 
	...
	function noStocks() {
		$CI =& get_instance();
		$outOfStocks = $CI->db->select("items.barcode, items.id, items.name,items.description, items.supplier_id, items.name, ordering_level.quantity")
				->from("items")
	...
	function low_stocks() {
		$CI =& get_instance();
		$running_outofstock = $CI->db->select("items.id, items.supplier_id, items.name, items.barcode, items.name,items.description, items.reorderingLevel, ordering_level.quantity")
				->from("items")
				->join("ordering_level", "ordering_level.item_id = items.id", "left") 
